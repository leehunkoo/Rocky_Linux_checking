...
def Check():
    result = {
        ...
        "remediation_cmd": (
            ...
            "# Step 3: login.defs 설정 수정\n"
            "sed -i 's/^PASS_MAX_DAYS.*/PASS_MAX_DAYS   90/g' /etc/login.defs && "
            "sed -i 's/^PASS_MIN_DAYS.*/PASS_MIN_DAYS   1/g' /etc/login.defs"
        ),
        "exception_guide": "[양호] pwquality 복잡성(Root 적용 포함) 설정 완료, PAM(system-auth/password-auth) 모듈 순서 정상, login.defs 패스워드 기간 정책이 준수된 경우"
    }
    ...
    summary_text = (
        ...
        f"[Step 3 login.defs]: {s3_lbl} (MAX={max_days}, MIN={min_days})"
    )
